...
export function loadConfig({ validate = true } = {}) {
  ...
  return {
    ...
    metaVerifyToken: provider === 'meta' ? get('META_VERIFY_TOKEN') : (process.env.META_VERIFY_TOKEN || ''),
    metaAppSecret: provider === 'meta' ? get('META_APP_SECRET') : (process.env.META_APP_SECRET || ''),
    metaGraphVersion: process.env.META_GRAPH_VERSION || 'v23.0',
    defaultMetaToken: provider === 'meta' ? get('META_PAGE_ACCESS_TOKEN') : (process.env.META_PAGE_ACCESS_TOKEN || ''),
