...
export function signedState(secret, ttlMinutes = 10) {
  const payload = Buffer.from(JSON.stringify({ nonce: crypto.randomBytes(18).toString('base64url'), exp: Date.now() + ttlMinutes * 60_000 })).toString('base64url');
  const signature = crypto.createHmac('sha256', secret).update(payload).digest('base64url');
  return `${payload}.${signature}`;
}

export function verifySignedState(value, secret) {
