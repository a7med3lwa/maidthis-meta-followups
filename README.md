# MaidThis Messenger and Instagram Follow-up Automation

A complete, policy-aware follow-up system for Facebook Messenger and Instagram conversations generated by Meta ads. Its primary mode is now a standalone **Meta OAuth** connection: click a button, sign in through Meta, approve access, and select the MaidThis Facebook Page and linked Instagram professional account. It remembers the highest MaidThis follow-up stage sent, ignores ordinary conversation when calculating the stage, pauses whenever a customer replies, resumes at the next stage after the sales conversation goes quiet, and prevents duplicate sends.

For the current MaidThis Railway deployment, follow [META-OAUTH-SETUP.md](META-OAUTH-SETUP.md). HighLevel and legacy direct-token adapters remain available as alternatives.
...
- Standalone Facebook Login, Page selection, and linked-Instagram discovery
- AES-256-GCM encryption of Page tokens before Supabase storage
- Existing-conversation history import through Meta or HighLevel
...
- Signed Meta webhooks and automatic Page subscription
...
- Responsive dark-first dashboard with a persistent light-mode option
...
```mermaid
flowchart TD
  U[Admin] -->|Facebook Login| O[Meta OAuth]
  O -->|encrypted Page token| A[Follow-up service]
  M[Messenger / Instagram] <-->|webhook and Send API| A
  A <--> S[(Supabase)]
...
5. Run `sql/003_meta_oauth.sql`.
6. In **Settings > API Keys**, create a new `sb_secret_...` secret key. Keep it server-side only. The publishable key is not used by this backend.

### 2. Choose the messaging connection

For the standalone login flow, use [META-OAUTH-SETUP.md](META-OAUTH-SETUP.md) and set `MESSAGING_PROVIDER=meta_oauth`.

If a HighLevel Private Integration token is available, [HIGHLEVEL-SETUP.md](HIGHLEVEL-SETUP.md) remains an alternative. Set `MESSAGING_PROVIDER=highlevel`.

Use the direct Meta instructions below only when you control the Page's Meta developer setup and intentionally set `MESSAGING_PROVIDER=meta`.
...
| `MESSAGING_PROVIDER` | `meta_oauth` for the login flow, `highlevel`, or legacy `meta` |
| `META_APP_ID` | App ID shown in Meta App Settings |
| `META_APP_SECRET` | Secret used for OAuth exchange and webhook validation |
| `TOKEN_ENCRYPTION_KEY` | 64-character private hex key used to encrypt Page tokens |
| `PRIVACY_CONTACT_EMAIL` | Email displayed on public policy pages |
| `HIGHLEVEL_API_TOKEN` | HighLevel Private Integration token; Railway only |
| `HIGHLEVEL_LOCATION_ID` | ID of the HighLevel sub-account containing the conversations |
| `META_VERIFY_TOKEN` | Random value you also enter in Meta's webhook screen |
| `META_PAGE_ACCESS_TOKEN` | Legacy direct-token mode only; not used by OAuth mode |
...
Complete verification and choose the message webhook fields. OAuth mode subscribes the selected Page to the app automatically. The service validates `X-Hub-Signature-256`; unsigned or incorrectly signed requests are rejected.
...
3. Click **Connect Facebook & Instagram**, complete Meta authorization, and select the MaidThis accounts.
4. Click **Import history** for each connected inbox. This scans known messages, detects the highest template stage in each conversation, and stores the messages idempotently.
5. Spot-check at least 20 conversations against Business Suite, especially stages 3, 7, and 9.
6. Allow the built-in scheduler to build the review queue. Sending remains paused.
7. Set **Global sending = Enabled** while keeping **Review required** selected.
8. Approve individual follow-ups for several days. Confirm replies cancel pending items and manual saved replies advance the correct stage.
9. When the decisions are consistently correct, change **Mode = Automatic**.
...
Use an internal/test conversation first. Verify this exact checklist:
